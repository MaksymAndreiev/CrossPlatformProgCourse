...
public class NumMethods {
    ...
    private static double meth (double x, double h, Evaluetable f){
        return 0.5*(f.evalf(x+h) - f.evalf(x-h))/h;
    }

    public static double der(double x, double tol, Evaluetable f){
        final int MAX = 100;
        double h = 0.1;
        ...
        do {
            h = 0.1*h;
            tmp = meth(x, h, f);
            ok = (abs(tmp-two) >= abs(two-one))||(abs(two-one)<tol);
            if (i > MAX){
                System.out.print("Забагато кроків обчисленнь");
                System.exit(-1);
            }
            i += 1;
            one = two;
    ...
    public static double findRoot(double appr, double eps, Evaluetable f){
    ...
    public static void main(String[] args){
        ...
        System.out.println("Числове значення sin'("+x+") = "+res);
        System.out.println("Символьне значення sin'("+x+") = "+cos(x));
        System.out.println("Абсолютна помилка = "+abs(res-cos(x)));
    }
}
